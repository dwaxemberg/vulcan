...
int HandleOps(unsigned char* programCode) {
	unsigned char regs[8] = { cpu.rB, cpu.rC, cpu.rD, cpu.rE, cpu.rH, cpu.rL, (char)cpu.rHL, cpu.rA };
	unsigned short shortRegs[5] = { cpu.rBC, cpu.rDE, cpu.rHL, cpu.rSP, cpu.rAF };
	unsigned char n;
...
#pragma region 16 bit loads
		...
		case 0x31: // SP
		{
					   unsigned char regIndex = (programCode[0] & 0xF0) >> 4; // get an index from the code value 
					   shortRegs[regIndex] = CHARS_TO_SHORT(programCode[1], programCode[2]);
					   return 3;
		}
		...
		case 0xF8:
		{
					 cpu.rHL = TO_USHORT(TranslateAddress(cpu.rSP + (signed)programCode[1]));
					 cpu.rF.Z = 0;
					 cpu.rF.N = 0;
					 //TODO: Figure out what to do with these flags!
					 cpu.rF.C = 0;
					 cpu.rF.H = 0;
					 return 2;
		}
		...
		case 0x08:
		{
					 cpu.rSP = CHARS_TO_SHORT(programCode[1], programCode[2]);
					 return 3;
		}
		...
		{
					   cpu.rSP -= 2;
					   TO_USHORT(TranslateAddress(cpu.rSP)) = cpu.rAF;
					   return 1;
		}
		case 0xC5: // BC
		case 0xD5: // DE
		case 0xE5: // HL
		{
					   unsigned char regIndex = ((programCode[0] & 0xF0) >> 4) - 12; // get an index from the code value 
					   cpu.rSP -= 2;
					   TO_USHORT(TranslateAddress(cpu.rSP)) = shortRegs[regIndex];
					   return 1;
		}
		...
		case 0xF1: // AF
		{
					   cpu.rAF = CHARS_TO_SHORT(TranslateAddress(cpu.rSP++), TranslateAddress(cpu.rSP++));
					   return 1;
		}
		case 0xC1: // BC
		case 0xD1: // DE
		...
		{
					   unsigned char regIndex = ((programCode[0] & 0xF0) >> 4) - 12; // get an index from the code value
					   shortRegs[regIndex] = CHARS_TO_SHORT(TranslateAddress(cpu.rSP++), TranslateAddress(cpu.rSP++));
					   return 1;
		}
